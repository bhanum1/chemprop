...
class MPNN(pl.LightningModule):
    ...
    def training_step(self, batch: TrainingBatch, batch_idx):
        ...
        l = self.criterion(preds, targets, mask, weights, lt_mask, gt_mask, temps, lnA_targets, EaR_targets)
    ...
    def _evaluate_batch(self, batch) -> list[Tensor]:
        bmg, V_d, X_d, targets, _, lt_mask, gt_mask, temps, lnA_targets, EaR_targets = batch
        ...
        return [
            metric(preds, targets, mask, None, lt_mask, gt_mask, temps, lnA_targets, EaR_targets) for metric in self.metrics[:-1]
        ]
